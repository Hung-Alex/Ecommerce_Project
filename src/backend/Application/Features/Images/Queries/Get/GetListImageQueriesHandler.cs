...
{
    public class GetListImageQueriesHandler : IRequestHandler<GetListImageQuery, Result<IEnumerable<CategoryDTO>>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private IMapper _mapper;
        public GetListImageQueriesHandler(IUnitOfWork unitOfWork, IMapper mapper)
        {
            _unitOfWork = unitOfWork;
            _mapper = mapper;
        }
        public async Task<Result<IEnumerable<CategoryDTO>>> Handle(GetListImageQuery request, CancellationToken cancellationToken)
        {
